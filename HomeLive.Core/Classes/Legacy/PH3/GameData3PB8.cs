 ...
 * https://github.com/kwsch/PKHeX/blob/e41797d0cbec195577a993e97e79e979c2e97ce3/PKHeX.Core/PKM/HOME/GameDataPB8.cs
 * GPL v3 License
...
/// </summary>
public sealed class GameData3PB8 : HomeOptional3, IGameDataSide3<PB8>, IGameDataSplitAbility, IPokerusStatus
{
    ...
    public GameData3PB8() : base(SIZE) { }
    public GameData3PB8(Memory<byte> data) : base(data) => EnsureSize(SIZE);
    public GameData3PB8 Clone() => new(ToArray());
    ...
    public void CopyTo(PB8 pk, PH3 pkh)
    {
        this.CopyTo(pk);
    ...
    public void CopyFrom(PB8 pk, PH3 pkh)
    {
        this.CopyFrom(pk);
        // Move Records are not settable in PB8; do not copy even if nonzero (illegal).
    ...
    public PB8 ConvertToPKM(PH3 pkh)
    {
        var pk = new PB8();
        pkh.CopyTo(pk);
        CopyTo(pk, pkh);
    ...
    public static GameData3PB8? TryCreate(PH3 pkh)
    {
        var side = GetNearestNeighbor(pkh);
        if (side is null)
            return null;

        var result = new GameData3PB8();
